...
function slp_holiday_calendar_shortcode($args){
...
#
	$cp_separator = ': ';
		...
		$cal =  'calendar_pickr_'. $id; 
		
		$datum		= get_post_meta($post->ID,'holidays',true);
	...
	$j(function() {
		var saved_dates = $j('#holidays_<?php echo $id; ?>').val().split(',');
		...
		{dateFormat: 'yyyy-m-d', firstDay:0,
		 multiSelect: 999, monthsToShow: 1, 
			...
			onSelect: function(dates) { 
				...
				$j('#holidays_<?php echo $id; ?>').val(fixdates);
			} 
	...
	<center><span>
	<input type="hidden" id="holidays_<?php echo $id; ?>" name="holidays" value="<?php echo $datum; ?>" />
...
add_shortcode('slp_holiday_calendar', 'slp_holiday_calendar_shortcode');	

/***************************** DATE PICK ADMIN ********************************
 *******************************************************************************/
 
 
 function slp_report_datepick_shortcode($args){
	define('cp_lang', 'en-GB');
 	define('datepick_package', get_stylesheet_directory_uri() . "/includes/admin/jquery.datepick.package");
 ...
 #
 	...
 	$j(function() {
 		...
 		$j("#<?php echo $cal; ?>").datepick(
 		{dateFormat: 'yyyy-m-d', firstDay:0,
 		 multiSelect: 0, monthsToShow: 1, 
		 	onDate: $j.datepick.noWeekends,
 			onSelect: function(dates) { 
 				fixdates = Array();
 				//f = $j.datepick.ATOM;
 				f = 'yyyy-m-d';
